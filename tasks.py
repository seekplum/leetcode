...
def check(ctx, job=4):
    """检查代码规范
    """
    ctx.run("pylint --rcfile=.pylintrc -j %s --output-format parseable src" % job,
            echo=True)
...
def unittest(ctx):
    """运行单元测试和计算测试覆盖率
    """
    ctx.run("export PYTHONPATH=`pwd`/src && pytest --cov=src tests", encoding="utf-8", pty=True,
            echo=True)
...
def coverage(ctx):
    """运行单元测试和计算测试覆盖率
    """
    ctx.run("export PYTHONPATH=`pwd`/src && "
